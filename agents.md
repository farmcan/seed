...
- 性能优先图谱 UI：`tools/video-dag-cytoscape.html`
- 画布/图谱库：`tools/vendor/elk.bundled.js`、`tools/vendor/cytoscape.min.js`
- Skills：
...
- DOM/ELK 画布使用 vendored `elkjs` layered layout 做自动分层布局；手写布局只能作为本地脚本加载失败的 fallback。Cytoscape 图谱入口用于性能验证，主画布只渲染轻量图谱，媒体放在右侧详情按需加载。
...
- Canvas lint：不要再手写主布局算法；主路径使用 vendored 成熟布局库，手写逻辑只允许作为 fallback 或小交互 glue。遇到卡顿优先切换到 Cytoscape/图谱引擎，而不是继续堆 DOM 卡片优化。
...
.venv/bin/seed export-video-dag-cytoscape-html --help
