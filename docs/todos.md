...
- [x] 建立“财报 + 研报”结构化技能工作流。
  ...
  - 新增 `first_principles` 字段（实际表示“商业本质结构化维度”），覆盖商业模式、营收逻辑、竞争格局与护城河、客户依赖、AI 替代风险、海外营收与出海进度、核心不确定性，并新增 `ecosystem_implications`（工具/平台、模型公司含义、算力/硬件信号、相关受益公司、传导不确定性）。对应 HTML 报告新增“商业本质（第一性视角）”下的产业传导启发卡片，用于从美图这类标的提炼对卖铲子与算力链条的参考。
