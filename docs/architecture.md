...
- `library/semantics/*.equity-research.json`：研报观点 ledger，记录 `viewpoint_events`、`first_principles`（商业模式/营收逻辑/竞争与护城河/客户依赖/AI 替代风险/海外进度）及 `ecosystem_implications`（工具厂、模型公司、算力/硬件链条启发），`notes_summary`、`source_gaps`、`open_questions`，以及风险与失效条件；进入财报 outlook 报告链路，不默认进入 video/creator DAG。
