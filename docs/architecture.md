...
- `domains/finance.py`：财经领域信号抽取、窗口汇总、可选行情补强和新闻 facts 上下文。输入单条 `video-semantics.md`，输出 `*.finance-signals.json`，每条信号优先返回 `viewpoint_events`（主对象）；输入多条 signals，输出 `*.finance-digest.json`；显式 ticker mapping 后可用 Stooq 日线生成 `*.finance-digest.priced.json`；`seed enrich-finance-news` 可把 `*.news-digest.json` 的 facts、industry impacts 和 market relevance 挂到事件级 `news_context`，只做事实引用；`seed build-finance-news-report` 把 news-context digest 渲染成卡片式 HTML 报告；`seed build-finance-outlook-report` 做标的级风险收益前瞻草案；`skills/finance-outlook-analyzer/SKILL.md` 约束 outlook 的 event 汇总口径、上下行映射、first principles 与风险收益边界；`seed distill-up-list` 在财经名单配置 `news_digest_paths` 时会自动补生成 news-context digest、news report 与 outlook 报告，并把它们链接进 UP 主页。所有观点都必须标记为创作者观点，不是 Seed 投资建议。
