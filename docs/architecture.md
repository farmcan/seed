...
seed build-finance-daily-brief <finance-digest...>
  -> 把多位财经 UP/创作者的窗口 digest 合成每日简报
  -> 每人一章 + 跨 UP 共识/分歧 + 方法论候选 + 证据缺口
  -> 输出 `library/distilled/*.finance-creator-daily-brief.json` 和 `library/reports/*.finance-creator-daily-brief.html`
...
| Research & statement ledger | 把研报观点和财报公开资料转成可核验 claim ledger，不替代交易建议 | research 报告文本、SEC filings artifact | `*.equity-research.json`, `*.financial-statement-review.json` | `domains/equity_research.py`, `skills/equity-research-report-analyzer/SKILL.md`, `skills/financial-statement-reviewer/SKILL.md` | `seed` report 命令、creator 财报上下文、research outlook 报告 | report 为主要消费端，当前不要求进 video/creator DAG | LLM 调用预留成本 | 不输出 buy/sell；保留证据边界、失效条件、open questions |
...
| 领域信号 | `seed extract-finance-signals`, `seed build-finance-digest`, `seed enrich-finance-prices`, `seed enrich-finance-news`, `seed build-finance-news-report`, `seed build-finance-daily-brief`, `seed build-finance-outlook-report`, `seed build-finance-business-analysis-report`, `seed run-video-pipeline --domain finance`, `seed run-creator-pipeline --domain finance` | `src/seed/domains/finance.py`, `src/seed/reports/finance_news.py`, `src/seed/reports/finance_daily.py`, `src/seed/reports/finance_outlook.py`, `src/seed/reports/finance_business_analysis.py`, `src/seed/reports/finance_outlook_chart.py`, `src/seed/reports/finance_outlook_assets.py`, `src/seed/skill_refs.py` | `library/semantics/*.finance-signals.json`, `library/distilled/*.finance-digest.json`, `*.finance-digest.priced.json`, `*.finance-digest.news-context.json`, `library/distilled/*.finance-creator-daily-brief.json`, 可选 `market_context` / `market_scenarios` / `consensus_diagnostics` / `research_methodology` / `user_value` / `company_assets` / `business_analysis`, `library/reports/*.finance-news-report.html`, `library/reports/*.finance-creator-daily-brief.html`, `library/reports/*.finance-outlook-report.html`, `library/reports/*.business-analysis.md`, `library/reports/*.business-analysis.html`, library/distilled/*.finance-outlook.json, video DAG / creator DAG finance 节点 |
...
| 研报与财报 statement ledger | `seed extract-equity-research-note`, `seed extract-equity-research-json`, `seed build-equity-research-report`, `seed parse-earnings --review-financial-statement` | `src/seed/domains/equity_research.py`, `src/seed/reports/equity_research.py`, `skills/equity-research-report-analyzer/SKILL.md`, `skills/financial-statement-reviewer/SKILL.md` | `library/notes/*.research-note.md`, `library/semantics/*.equity-research.json`, `library/distilled/*.financial-statement-review.json`, `library/reports/*.equity-research-report.html` |
...
- `domains/finance.py`：财经领域信号抽取、窗口汇总、可选行情补强和新闻 facts 上下文。输入单条 `video-semantics.md`，输出 `*.finance-signals.json`，每条信号优先返回 `viewpoint_events`（主对象）；输入多条 signals，输出 `*.finance-digest.json`；显式 ticker mapping 后可用 Stooq 日线生成 `*.finance-digest.priced.json`；`seed enrich-finance-news` 可把 `*.news-digest.json` 的 facts、industry impacts 和 market relevance 挂到事件级 `news_context`，只做事实引用；`seed build-finance-news-report` 把 news-context digest 渲染成卡片式 HTML 报告；`seed build-finance-daily-brief` 消费多个 creator/window finance digest，输出每人一章、跨 UP 共识/分歧、方法论候选和证据缺口，适合回答“今天财经 UP 都在说什么”；`seed build-finance-outlook-report` 做标的级风险收益前瞻草案，若 digest 提供 `market_context` / `market_scenarios`，则优先用真实行情、52 周区间、分析师目标价、财报/政策/产品催化构建上行/下行空间，事件后验只作为复核；`seed build-finance-business-analysis-report` 消费 `*.finance-digest*.json` 或 `*.finance-outlook.json`，把 `business_analysis`、`first_principles`、竞品状态和情景表渲染为 `*.business-analysis.md/html`，用于回答主营业务、收入来源、市场空间、竞争压力和后续股价路径；报告还会生成 `user_value`、`consensus_diagnostics`、`research_methodology` 与可选 `company_assets`，用于先回答普通用户“这份报告有什么用”、再展示目标价分歧、数据覆盖度、成熟研报方法论映射、公司 logo/产品图/核心产品名和交付自检；若 `market_context.historical_prices` 存在或能按 ticker 补强，报告默认用约 3 年真实日线 OHLC 渲染 K 线，写入 `historical_price_coverage`、覆盖起止日期和交易日数，并用 vendored TradingView Lightweight Charts 展示当前价/目标价水平线、`historical_events/chart_events` 历史事件 marker、未来关键时间点事件标记和非 K 线情景路径；未来情景路径按日采样，图下同轴事件时间线用同一日期范围标出“当前 / 最后一根真实 K 线”、未来观察窗、持续窗口事件和未来单点事件的关注内容/潜在影响/多情景触发，不生成伪未来每日 K 线、粗箭头或大面积扇形；历史事件可来自财报、产品、研报、政策、宏观/战争/油价或 AI 模型能力节点，但必须有来源链接，且只作为可能解释变量，不写成确定因果；视觉资产只能来自官方、IR、产品页、媒体包或明确可引用来源，并保留 source URL/使用限制，不把图片当作财务事实；`skills/finance-outlook-analyzer/SKILL.md` 约束 outlook 的市场检索、event 汇总口径、上下行映射、first principles、业务分析、视觉资产与风险收益边界；`seed distill-up-list` 在财经名单配置 `news_digest_paths` 时会自动补生成 news-context digest、news report 与 outlook 报告，并把它们链接进 UP 主页；`run-creator-pipeline --domain finance` 后处理会优先基于窗口 digest 生成 `*.finance-outlook.json` 与对应 report。所有观点都必须标记为创作者观点，不是 Seed 投资建议。
...
- `domains/equity_research.py`：研报观点与财报 statement 结构化。`extract-equity-research-note` 产出 `*.research-note.md`，`extract-equity-research-json` 产出 `*.equity-research.json`，`parse-earnings --review-financial-statement` 产出 `*.financial-statement-review.json`。
...
- `skills/equity-research-report-analyzer/SKILL.md`：研报观点结构化模板，用于 `extract-equity-research-note` / `extract-equity-research-json` / `build-equity-research-report`，定义观点事件与证据映射边界。
- `skills/financial-statement-reviewer/SKILL.md`：财报 statement 审阅模板，用于 `parse-earnings --review-financial-statement`，约束口径、`evidence_gaps`、`inconsistencies` 与 `open_questions`。
- `skills/finance-outlook-analyzer/SKILL.md`：财经观点前瞻草案模板，用于 `build-finance-outlook-report`，约束 event-level 的价格映射、风险收益和可核验边界。
...
- `library/notes/*.research-note.md`：研报文本抽取稿，保留结构化元信息与可核验 source sections，用于后续 ledger 生成。
...
- `library/semantics/*.equity-research.json`：研报观点 ledger，记录 `viewpoint_events`、`first_principles`（商业本质：商业模式/营收逻辑/竞争与护城河/客户依赖/AI 替代风险/海外进度）及 `ecosystem_implications`（工具厂、模型公司、算力/硬件链条启发），`notes_summary`、`source_gaps`、`open_questions`，以及风险与失效条件；进入财报 outlook 报告链路，不默认进入 video/creator DAG。
...
- `library/distilled/*.finance-digest.news-context.json`：在 finance digest 基础上为 `viewpoint_events` 追加 `news_context`，引用 news digest 的 facts、source URLs、industry impacts、market relevance、source gaps 和 open questions；可附加 `market_context`、`market_scenarios` 和 `company_assets`，记录真实行情、近期 OHLC、目标价、52 周区间、未来催化点、公司 logo/产品图/核心产品名和来源引用；只作为事实上下文，不改变创作者观点，也不做交易建议。
- `library/distilled/*.finance-creator-daily-brief.json`：财经 UP 每日/窗口化简报 artifact，消费多个 finance digest，聚合 creator sections、跨 UP consensus/conflict、methodology signals、risk flags 和 evidence gaps；不直接进入 video DAG，后续可作为 creator DAG / UP 主页的报告节点。
- `library/distilled/*.financial-statement-review.json`：财报 statement 审阅输出，按 statement scope 输出 `income_statement` / `balance_sheet` / `cash_flow` / `segment_data` 快照与质量控制字段。
- `library/reports/*.finance-news-report.html`：面向人工阅读的财经观点新闻事实报告，按热点、UP、视频、观点事件、facts 对照、行业影响、source gaps 和 open questions 展示。
- `library/reports/*.finance-creator-daily-brief.html`：面向人工阅读的财经 UP 每日简报，按每位 UP 一个章节展示观点、样本、风险和证据缺口，并提供跨 UP 共识/分歧与方法论候选；由 `seed build-finance-daily-brief` 生成。
...
- `library/reports/*.equity-research-report.html`：研报观点与财务 statement 结合报告，面向人工阅读的前瞻与风险汇总视图。
